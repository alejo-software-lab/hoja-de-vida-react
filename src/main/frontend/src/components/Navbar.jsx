        ...
        <div className="flex items-center justify-between h-16">
          <div className="flex-shrink-0 flex items-center gap-2">
            <a href="#inicio" className="flex items-center gap-2 group">
              <img src="/logo.png" alt="Alejo Software Labs" className="h-8 w-auto" />
              <span className="font-bold text-xl tracking-tight text-indigo-600 dark:text-indigo-400">Alejo Software Labs</span>
            </a>
          </div>
          ...
          <div className="flex items-center gap-2 md:hidden">
            <a href={CV_LINK} target="_blank" rel="noreferrer" className="p-2 rounded-xl bg-indigo-600 text-white hover:bg-indigo-700 transition-all duration-200 hover:scale-110">
              <Download className="w-5 h-5" />
            </a>
            <button onClick={() => setDarkMode(!darkMode)} className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors duration-200 hover:scale-110">
              {darkMode ? <Sun className="w-5 h-5 text-amber-500" /> : <Moon className="w-5 h-5 text-indigo-600" />}
            </button>
            <button onClick={() => setIsOpen(!isOpen)} className="p-2 rounded-xl text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors hover:scale-110">
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 mx-4 rounded-2xl glass shadow-xl mt-2 border border-slate-200/50 dark:border-slate-800/50">
            {navLinks.map((link) => (
              <a key={link.name} href={link.href} onClick={() => setIsOpen(false)}
                className="block px-4 py-3 rounded-xl text-base font-medium text-slate-700 dark:text-slate-300 hover:bg-indigo-50 dark:hover:bg-indigo-950/50 hover:text-indigo-600 dark:hover:text-indigo-400 transition-all duration-200">{link.name}</a>
            ))}
