        ...
        <div className="relative border-l-2 border-indigo-100 dark:border-indigo-900/50 ml-4 md:ml-32 space-y-12">
          {experiences.map((exp, index) => (
            <div key={index} className="relative pl-8 md:pl-12 group">
              ...
              {/* Timeline dot */}
              <div className="absolute -left-[9px] top-1.5 w-4 h-4 rounded-full bg-white dark:bg-slate-950 border-4 border-indigo-500 dark:border-indigo-400 group-hover:scale-125 transition-transform duration-350 z-10"></div>
              ...
              {/* Card Container */}
              <div className="p-6 sm:p-8 rounded-3xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-850 hover:border-indigo-300 dark:hover:border-indigo-900/40 shadow-sm hover:shadow-md transition-all duration-300">
                ...
                <div className="flex flex-wrap gap-2">
                  {exp.skills.map((skill, sIdx) => (
                    <span
                      key={sIdx}
                      className="px-3 py-1 rounded-xl bg-indigo-50/50 dark:bg-indigo-950/30 border border-indigo-100/50 dark:border-indigo-900/40 text-indigo-650 dark:text-indigo-400 text-xs font-semibold"
                    >
                      {skill}
                    </span>
                  ))}
